...
Token *addTkText(int code, char* text)
{
...
void regexCheck(const char *pch, char* text, Token* tk)
{
    regex_t regex;
    int reti;
    ...
    for (;;)
    {
        ...
        default:
        {
            ...
            if (isalpha(*pch) || *pch == '_')
            {
                ...
                {
		  tk = addTkText(ID, text);
                    tk->text = text;
                }
            }
            else
            {
	      for (start = pch; strchr(",;)([]{}",*pch)==0; pch++){printf("%c\n", *pch);}
	       if(start!=pch)pch--;
	      char *text = extract(start, pch);
	      regexCheck(pch, text, tk);
	       pch++;
            }
        }
        }
